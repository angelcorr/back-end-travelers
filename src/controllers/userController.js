...
userController.register = async (req, res) => {
  try {
    ...
    const accessToken = jwt.sign({ identifier }, process.env.SECRET_TOKEN, {
      expiresIn: 60 * 60 * 24
    });
    
    return res.json({ auth: true, user: user, accessToken: accessToken });
  } catch (error) {
...
userController.auth = async (req, res) => {
  try {
    ...
    const identifier = user.id;
    console.log('mail es', mail);
  
    if (!user) {
      return res.status(401).json({ auth: false, massaje: 'user not found' });
    }
